...
Add unit tests close to the code they verify using `#[cfg(test)]` modules. Prefer deterministic filesystem tests that create and clean up temporary directories. Coverage must stay at or above 80%; verify with `rtk scripts/check-coverage.sh` after meaningful changes. After the final code change, always build the release binary with `rtk cargo build --release`. When changing CLI behavior, verify both `rtk cargo test` and a targeted manual run such as `rtk cargo run -- --debug-usage`, `rtk cargo run -- --waybar`, `rtk cargo run -- --install-waybar-config`, or `rtk target/release/codex-switch --remove-link`. Waybar config installation must preserve unrelated user settings and write backups before editing files under `~/.config/waybar`.
